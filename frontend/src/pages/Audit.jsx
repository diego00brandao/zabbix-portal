...
function formatDetails(details) {
  ...
  return Object.entries(details).map(([key, val]) => ({ key, oldval: val?.old ?? '', newval: val?.new ?? val ?? '' }));
}
...
function StatCard({ icon, label, value, sub, color, alert }) {
  return (
    <div style={{ background: alert ? `${color}10` : 'var(--bg-card)', border: `1px solid ${alert ? color+'35' : 'var(--border)'}`, borderRadius: '10px', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '4px', position: 'relative', overflow: 'hidden' }}>
      {alert && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: color }} />}
...
export default function Audit() {
  ...
  const filtered = useMemo(() => enrichedLogs.filter(l => {
    ...
    return true;
  }), [enrichedLogs, showAll, search, actionFilter, riskFilter]);
  ...
  const analytics = useMemo(() => {
    ...
    return { todayCount: todayLogs.length, criticalCount: criticalLogs.length, topUser, actionCount, resourceCount, topObjects, hourDist, peakHour, uniqueUsers: Object.keys(userCount).length };
  }, [enrichedLogs]);
    ...
    <div style={S.root}>
      <div style={S.header}>
        <div>
          ...
          <p style={S.sub}>Rastreabilidade completa · hosts, triggers, templates, itens</p>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:'12px', marginBottom:'20px' }}>
        <StatCard icon="📋" label="Total de Eventos" value={enrichedLogs.length} sub={`${analytics.todayCount} hoje`} color="var(--text-accent)" />
        <StatCard icon="🔴" label="Ações Críticas"   value={analytics.criticalCount} sub="remoções e desativações" color="#ff5757" alert={analytics.criticalCount>0} />
        <StatCard icon="👥" label="Usuários Ativos"  value={analytics.uniqueUsers} sub={analytics.topUser?`+ ativo: ${analytics.topUser[0]}`:''} color="#4a9eff" />
      </div>
