...
export default function Dashboard() {
  ...
  return (
    <div style={styles.root}>
      ...
      {/* Itens com erro */}
      {(extras?.itemsWithError?.length > 0) && (
        <div style={{ ...styles.card, marginTop: '16px' }}>
          <div style={styles.cardHeader}>
            <span style={styles.cardTitle}>⚠ Itens com Erro de Coleta</span>
            <span style={{ fontSize: '11px', color: 'var(--red)', fontFamily: 'var(--font-mono)' }}>{extras.itemsWithError.length} itens</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
            {extras.itemsWithError.slice(0, 6).map((item, i) => (
              <div key={i} style={{ padding: '8px 12px', background: 'var(--red-dim)', border: '1px solid rgba(255,87,87,0.2)', borderRadius: 'var(--radius)' }}>
                <div style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</div>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginTop: '2px' }}>{item.hosts?.[0]?.name || '—'}</div>
                <div style={{ fontSize: '10px', color: 'var(--red)', marginTop: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.error}</div>
              </div>
            ))}
          </div>
        </div>
