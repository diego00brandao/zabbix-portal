...
export default function Items() {
  ...
  const filtered = items.filter(item => {
    ...
    const matchType = typeFilter === 'all' || (typeFilter === 'query' && item.isQuery);
    const matchStatus =
      statusFilter === 'all' ||
    ...
    <div style={styles.root}>
      ...
      <div style={styles.filters}>
        ...
        <div style={styles.tabs}>
          {[['all', 'Todos'], ['query', '◎ Queries SQL']].map(([val, lbl]) => (
      ...
      <div style={styles.table}>
        ...
        {loading ? (
          ...
          filtered.slice(0, 300).map(item => (
            <div key={item.itemid} style={{ ...styles.row, ...(item.isQuery ? styles.rowQuery : {}), ...(item.status === '1' ? { opacity: 0.6 } : {}) }} className="animate-in">
              <div>
      ...
      {filtered.length > 300 && (
        <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '12px', padding: '12px' }}>
          Exibindo 300 de {filtered.length} itens. Use o filtro para refinar.
        </p>
      )}
    </div>
  );
...
const styles = {
  ...
  queryPre: { fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--purple)', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: '4px', padding: '4px 8px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', margin: 0 },
