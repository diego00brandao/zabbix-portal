...
function HealthReport({ health, onDownloadJSON, onDownloadCSV, onDownloadExcel, onTemplateClick }) {
  ...
  const { summary, host, alerts, queryItems } = health;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {queryItems.length > 0 && (
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 'var(--radius)', padding: '14px' }}>
          <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--purple)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>◎ Itens Database / Queries SQL</div>
          {queryItems.map(item => (
            <div key={item.itemid} style={{ marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: 500, marginBottom: '4px' }}>{item.name}</div>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: '4px' }}>Chave: {item.key_} · Intervalo: {item.delayFormatted}</div>
              {item.params && <pre style={styles.queryPre}>{item.params}</pre>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {loading ? (
        ...
        <>
          <ChartCard title="CPU Utilization %" series={trends.cpu.series} color="#ff9f43" analysis={trends.cpu.analysis} />
          <ChartCard title="Memory Utilization %" series={trends.memory.series} color="#4a9eff" analysis={trends.memory.analysis} />
          {trends.disk.map((d, i) => (
