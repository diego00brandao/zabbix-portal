...
const NAV_SECTIONS = [
  {
    ...
    items: [
      ...
      { to: '/item-errors',  icon: '⚠', label: 'Erros de Coleta',roles: ['admin','manager','viewer'] },
      { to: '/maintenance',  icon: '⊙', label: 'Manutenções',   roles: ['admin','manager','viewer'] },
    ]
  },
  {
    label: 'CONFIGURAÇÃO',
...
export default function Layout() {
  const { user, logout, selectedConnection, setNeedsConnectionSelect } = useAuth();
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <aside style={{ ...styles.sidebar, width: collapsed ? '56px' : '220px' }}>
        <div style={styles.sidebarHeader}>
          ...
          {!collapsed && <div>
            <div style={styles.sidebarTitle}>Observability Portal</div>
            {selectedConnection && <div style={{ fontSize:'10px', color:'var(--gold)', fontFamily:'var(--font-mono)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'140px' }}>⇌ {selectedConnection.name}</div>}
          </div>}
        ...
        <nav style={styles.nav}>
          {NAV_SECTIONS.map(section => (
            <div key={section.label}>
              {!collapsed && <div style={styles.navDivider}>{section.label}</div>}
          ...
          {(user?.role === 'admin') && (
            <div>
              ...
              {collapsed && <div style={{ ...styles.navDivider, textAlign: 'center' }}>─</div>}
              <NavLink to="/admin/users" style={({ isActive }) => ({ ...styles.navItem, ...(isActive ? styles.navItemActive : {}) })}><span style={styles.navIcon}>⊙</span>{!collapsed && <span>Usuários</span>}</NavLink>
              <NavLink to="/admin/areas" style={({ isActive }) => ({ ...styles.navItem, ...(isActive ? styles.navItemActive : {}) })}><span style={styles.navIcon}>⊞</span>{!collapsed && <span>Áreas</span>}</NavLink>
              <NavLink to="/admin/connections" style={({ isActive }) => ({ ...styles.navItem, ...(isActive ? styles.navItemActive : {}) })}><span style={styles.navIcon}>⇌</span>{!collapsed && <span>Conexões</span>}</NavLink>
            </div>
          )}
        </nav>
