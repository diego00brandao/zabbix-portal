...
router.get('/hosts', authMiddleware, async (req, res) => {
  ...
  const data = hosts.map(h => ({
    ...
    templates: h.parentTemplates?.map(t => t.name).join(', ') || '',
  }));
...
router.get('/items', authMiddleware, async (req, res) => {
  ...
  const hostItems = (await zabbix.getItems(groupIds)).map(i => ({...i, query: i.params||''}) );
  let items = hostItems;
  ...
  if (format === 'csv') {
    ...
    return res.send(toCSV(items, [
      ...
      { fn: row => row.params || row.query || '', label: 'Query/Params' },
    ]));
