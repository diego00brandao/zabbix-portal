...
async function getAllTriggers(groupIds) {
  ...
  if (groupIds && groupIds.length > 0) params.groupids = groupIds;
  return call('trigger.get', params, true, 180);
}
...
async function getItems(groupIds, searchQuery = '') {
  const params = {
    ...
    selectHosts: ['host', 'name'],
    webitems: true, filter: {}, limit: 500,
  };
  if (groupIds && groupIds.length > 0) params.groupids = groupIds;
  if (searchQuery) params.search = { name: searchQuery, key_: searchQuery };
  const items = await call('item.get', params, true, 180);
  return items.map(item => ({
    ...item,
    isQuery: item.type === '11' || item.key_?.startsWith('db.odbc') || item.key_?.startsWith('db.query'),
    typeLabel: getItemTypeLabel(item.type),
...
async function getHostTrends(hostId, period = '1d', fromDate = null, toDate = null) {
  ...
  const [items, memItems, diskItems] = await Promise.all([
  ...
  const [cpuData, memData, diskData] = await Promise.all([fetchData(cpuIds), fetchData(memIds), fetchData(diskIds)]);
  ...
  return {
    ...
    disk: diskSeries, generatedAt: new Date().toLocaleString('pt-BR'),
  };
}
...
async function getAuditLogEnriched(timeFrom, timeTill) {
  try {
    ...
    const itemLogs    = logs.filter(l => String(l.resourcetype) === '15');
    const triggerLogs = logs.filter(l => String(l.resourcetype) === '13');
